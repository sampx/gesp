...
must_haves:
  ...
  key_links:
    - from: "projects/gesp/packages/backend/src/routes/debug.ts"
    ...
    - from: "projects/gesp/packages/backend/src/index.ts"
      to: "projects/gesp/packages/backend/src/routes/debug.ts"
      via: "import + mount"
...
From projects/gesp/packages/backend/src/routes/auth.ts (reference for route structure):
```typescript
...
<task type="auto">
  ...
  <files>projects/gesp/packages/backend/src/routes/debug.ts</files>
  <read_first>
    1. projects/gesp/packages/backend/src/routes/auth.ts — 现有路由模式参考
    2. projects/gesp/packages/backend/src/utils/response.ts — success/error 函数
    3. projects/gesp/packages/backend/AGENTS.md — 禁止 console.log，但此文件是开发工具，HTML 内联不涉及 logger
  </read_first>
  <action>
    创建 `projects/gesp/packages/backend/src/routes/debug.ts`，内容如下：
...
<body>
  ...
  <verify>
    <automated>grep -q "app.get.*debugHtml" projects/gesp/packages/backend/src/routes/debug.ts && grep -q "export default app" projects/gesp/packages/backend/src/routes/debug.ts && echo "PASS"</automated>
  </verify>
  <done>
    - debug.ts 文件存在
...
<task type="auto">
  ...
  <read_first>
    1. projects/gesp/packages/backend/src/index.ts — 当前挂载 auth routes 的模式
    2. projects/gesp/packages/backend/src/routes/debug.ts — 上一步创建的 debug 路由
  </read_first>
  <action>
    在 `projects/gesp/packages/backend/src/index.ts` 中：
  ...
  <verify>
    <automated>grep -q "import debugRoutes" projects/gesp/packages/backend/src/index.ts && grep -q "app.route('/debug', debugRoutes)" projects/gesp/packages/backend/src/index.ts && grep -q "ENABLE_DEBUG" projects/gesp/packages/backend/src/index.ts && echo "PASS"</automated>
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>projects/gesp/packages/backend/src/__tests__/debug-route.test.ts</files>
  <read_first>
    1. projects/gesp/packages/backend/src/__tests__/auth-middleware.test.ts — 现有测试模式参考
    2. projects/gesp/packages/backend/src/routes/debug.ts — debug 路由实现
  </read_first>
  <action>
    创建 `projects/gesp/packages/backend/src/__tests__/debug-route.test.ts`：
