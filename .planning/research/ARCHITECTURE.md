...
// projects/gesp/packages/backend/src/ellamaka/assessor.ts
...
```typescript
// projects/gesp/packages/backend/src/db/lance/schema.ts
...
```typescript
// projects/gesp/packages/backend/src/db/lance/retrieval.ts
...
```typescript
// projects/gesp/packages/backend/src/server/routes/ai/assessment.ts
...
```typescript
// projects/gesp/packages/backend/src/server/middleware/auth.ts
...
3. `projects/gesp/packages/backend/db` — SQLite 表结构、LanceDB 初始化（依赖：shared）

**Phase 2: 后端核心（业务层）**
4. `projects/gesp/packages/backend/service` — 业务服务（依赖：db, sdk）
5. `projects/gesp/packages/backend/ellamaka` — SDK 代理层 + SSE 转发（依赖：service, sdk）
6. `projects/gesp/packages/backend/server` — Hono 路由、中间件（依赖：ellamaka, service）
