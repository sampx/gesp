...
**Updated:** 2026-04-25（hono-openapi 1.x + zod v4 + Scalar UI）
...
| **OpenAPI** | hono-openapi | 1.3.x | 从路由生成 OpenAPI 规范，支持 SDK 生成。1.x 使用 Standard Schema 协议，配套 zod v4。 |
| **API Docs UI** | @scalar/hono-api-reference | 0.10.x | 现代 API 调试界面，挂载 /api/doc/ui。优于 Swagger UI。 |
| **Validation** | Zod | 4.x | 类型安全校验，v4 原生支持 OpenAPI schema 生成（不再需要 zod-openapi）。 |
