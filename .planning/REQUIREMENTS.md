...
- [x] **AUTH-05**: 所有用户可登出，登出后清除会话并跳转登录页
- [x] **AUTH-06**: 学员和教员可修改自己的密码（需验证旧密码）
- [x] **AUTH-07**: 管理员可修改自己的密码（需验证旧密码）

### Knowledge Base

- [x] **KNOW-01**: 系统以结构化数据存储 GESP 1-8 级课程大纲（C++，后续可扩展 Python）
- [x] **KNOW-02**: 系统存储历年真题及元数据（级别、主题、难度）
- [x] **KNOW-03**: 系统为知识点和题目提供向量检索（LanceDB）
- [x] **KNOW-04**: 管理员可查看和编辑知识库内容
- [x] **KNOW-05**: 管理员可添加新题目并进行元数据标签标注

### Authentication UI (Phase 2.1 新增)

- [x] **UI-AUTH-01**: 学员和教员可通过用户名+密码+确认密码自助注册，注册成功自动登录跳转对应角色首页。用户名 3-20 字符、密码 6+ 字符，表单实时验证+错误提示。登录页需提供"去注册"链接(管理员不可自助注册)
- [x] **UI-AUTH-02**: 学员\教员和管理源界面的导航区域均有登出按钮/入口，点击后清除会话跳转登录页
- [x] **UI-AUTH-03**: 所有用户设置页面，可修改密码（旧密码+新密码+确认新密码），修改成功后提示并保持登录
...
- [x] **ASSESS-02**: 题目覆盖 GESP 1-8 级课程范围（各级别定义的主题）
- [x] **ASSESS-03**: 系统自动为学员答案评分（客观题 + 编程题）
- [x] **ASSESS-04**: 初始测评完成后确定学员起始级别（5-10 道题）
- [x] **ASSESS-05**: 测评进度增量保存（支持中断后恢复）
...
- [x] **ADMIN-05**: 管理员可管理学员账号（查看列表、禁用/启用账号、重置学员密码）
- [x] **ADMIN-06**: 管理员可通过管理端界面添加教员账号（用户名+密码）
- [x] **ADMIN-07**: 管理员可通过管理端界面添加学员账号（用户名+密码）
...
| ASSESS-02 | Phase 3 | Complete | ellamaka assessor + gesp SDK |
